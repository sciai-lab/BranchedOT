...
def check_full_tree_topo(topo, supply_arr, demand_arr):
    """
    ...
    assert all_idx_list == list(np.arange(- (num_bps), len(supply_arr) + len(demand_arr))), \
        f"mistake in labelling: {all_idx_list, list(np.arange(-(num_bps), len(supply_arr) + len(demand_arr)))}"

    # check if topology is a full tree topology:
    check_list = [(num_bps == len(supply_arr) + len(demand_arr) - 2)]  # check number of bps

    # check degree of bps:
    for bp in list_bp_idx:
        check_list.append(topo.degree(bp) == 3)

    return np.asarray(check_list).all()
...
def iterative_geometry_solver(topo, supply_arr, demand_arr, coords_sources, coords_sinks, al,
    ...
    assert len(supply_arr) == coords_sources.shape[0], "mismatch in number of sources regarding supply_arr " \
                                                       "and coords_sources!"
    assert len(demand_arr) == coords_sinks.shape[0], "mismatch in number of sinks regarding demand_arr " \
                                                     "and coords_sinks!"
