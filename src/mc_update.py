...
from helper_fcts import generate_random_bot_problem, eucl_dist, dist_point_segments

def kernel(dist_arr, width=1.):
    p = np.exp(-dist_arr**2/(width * np.min(dist_arr))**2)
    return p / np.sum(p)
...
def monte_carlo_step(topo, sample_edge_list, cost, coords_arr, bot_problem_dict, temperature, kernel_width=1.):
    """
    ...
    dist_child_edges = dist_point_segments(connector_coords, edges_arr[:, :dim], edges_arr[:, dim:])
    dist_propabilities = kernel(dist_child_edges, width=kernel_width)
    ...
    # new_cost, coords_iter = iterative_bot_solver(topo, supply_arr, demand_arr, coords_sources, coords_sinks, al,
    #                                          relative_improvement_threshold=1e-6, min_iterations=-1,
    #                                          max_iterations=1000,
    #                                          plot=False, title="", fov=None, save=False, save_name="img")
    # use faster solver:
    _, new_cost, coords_iter, _ = fast_optimize(topo, supply_arr, demand_arr, coords_sources, coords_sinks, al,
